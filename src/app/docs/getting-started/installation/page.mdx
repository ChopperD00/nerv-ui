...
NERV-UI is a typed React component library for original command-center-inspired interfaces. It is built with Tailwind CSS, Framer Motion, and TypeScript.
...
The repository contains one canonical Agent Skills-compatible source at
[`skills/nerv-ui`](https://github.com/mdrbx/nerv-ui/tree/master/skills/nerv-ui).
Codex can discover it from the repository’s `.agents/skills` symlink. For a
user-level Codex installation, copy the directory into `~/.codex/skills/nerv-ui`:
...
The skill guides an agent to install the published package, use verified exports,
and check accessibility and reduced motion. Claude Code discovers the same
canonical source through `.claude/skills/nerv-ui`. Hermes Agent users can copy
it to `~/.hermes/skills/nerv-ui` and verify discovery with `hermes skills list`.
